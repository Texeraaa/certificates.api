...
export class EventPaperController {
  ...
  @ApiBody({
    type: CreateEventPaperDTO,
    description: 'Dados para criar um novo papel de evento',
  })
  @ApiResponse({
    status: 201,
  ...
  async createEventPaper(@Body() createEventPaperDTO: CreateEventPaperDTO) {
    const event =
      await this.eventPaperService.createEventPaper(createEventPaperDTO);
    return {
      status: 201,
      message: 'Papel de evento criado com sucesso',
      return: {
  ...
  @ApiParam({
    name: 'id',
    type: String,
    description: 'ID do papel de evento a ser deletado',
  })
  @ApiResponse({
    status: 200,
